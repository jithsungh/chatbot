...
class ApiService {
  ...
  async getUploadedFiles() {
    return this.client.get("/api/admin/upload/list");
  }
  // User query method
  async sendUserQuery(query, userId) {
    return this.client.post("/api/user/query/", {
      query,
      userid: userId,
    });
  }
  ...
  async uploadAdminFile(file, department) {
    const formData = new FormData();
    formData.append("file", file);

    return this.client.post(`/api/admin/upload/${department}`, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  ...
  async uploadAdminText(text, department) {
    return this.client.post(`/api/admin/upload/${department}`, {
      text: text,
    });
  }
